...
<head>
    ...
    <title>Home - List Counselor</title>
        ...
        @foreach ($counselors as $counselor)
            <div class="appointment-card">
                <div class="appointment-info">
                    <p>{{ $counselor->name }}</p>
                    <p>Active: {{ $counselor->active_days }}</p>
                </div>
                <div class="button-group">
                    <a href="{{ route('counselors.edit', $counselor->id) }}">
                        <button style="background-color: rgb(45, 177, 33); color: white;">Edit</button>
                    </a>
                    <form action="{{ route('counselors.destroy', $counselor->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus counselor ini?')">
                        @csrf
                        @method('DELETE')
