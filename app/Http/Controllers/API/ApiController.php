...
class ApiController extends Controller
{
    ...
    /**
     * Melakukan reschedule booking yang sudah ada.
     * Sesuai dengan rute: POST /booking/{id}/reschedule
     *
     * @param Request $request
     * @param int $id ID booking yang akan di-reschedule.
     * @return \Illuminate\Http\JsonResponse
     */
    public function rescheduleBooking(Request $request, $id)
    {
        try {
            ...
            if ($booking->user_id !== Auth::id()) {
                return response()->json(['message' => 'Anda tidak berhak me-reschedule booking ini.'], 403);
            }

            if ($booking->status !== 'booked') {
                return response()->json(['message' => 'Booking ini tidak bisa di-reschedule karena statusnya bukan "booked".'], 400);
            }

            $jadwalLama = Jadwal::find($booking->jadwal_id);
            $jadwalBaru = Jadwal::find($validatedData['jadwal_id_baru']);

            if (!$jadwalLama || !$jadwalBaru) {
                return response()->json(['message' => 'Satu atau kedua jadwal tidak ditemukan.'], 404);
            }

            if ($jadwalBaru->status !== 'available' || Carbon::parse($jadwalBaru->hari)->isPast()) {
                return response()->json(['message' => 'Jadwal baru tidak tersedia atau sudah berlalu.'], 409);
            }

            if ($jadwalLama->id === $jadwalBaru->id) {
                return response()->json(['message' => 'Jadwal baru tidak boleh sama dengan jadwal yang sudah ada.'], 400);
            }

            $jadwalLama->status = 'available';
            $jadwalLama->save();

            $booking->jadwal_id = $validatedData['jadwal_id_baru'];
            $booking->tanggal = $jadwalBaru->hari;
            ...
            return response()->json([
                'message' => 'Gagal me-reschedule booking.',
                'error' => $e->getMessage(),
            ], 500);
        }
    }
